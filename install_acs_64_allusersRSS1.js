...
var thisVersion = "January 24, 2023"
var printhelp = "no";
...
// Create short-cut for Run Sql Stetments
function rssShortcut(prod_path) {
  install_log.writeline(">>rssShortcut("+prod_path+")");	
  install_log.writeline(">>>Creating shortcut for Run Sql Statements");
  Shel99 = new ActiveXObject("WScript.Shell");
  //install_log.writeline(">>>link3.TargetPath : "+link3.TargetPath);
  link3 = Shel99.CreateShortcut(DesktopPath + "\\ACS Run Sql Statements.lnk");
  install_log.writeline(">>>link3 : "+link3);
  link3.Arguments = prod_path + win_exe + " /plugin=rss";
  link3.Description = "IBM i Access Client Solutions - Run Sql Statements";
  link3.HotKey = "CTRL+ALT+SHIFT+S";
  install_log.writeline(">>>prod_path : "+prod_path);
  install_log.writeline(">>>win_exe : "+win_exe);
  //link3.IconLocation = prod_path + win_exe + ",0";
  //\\fs1\apps\AS400\IBM\ACS\IBMiAccess_v1r1\ibmiaccess.ico
  link3.IconLocation = source+"\\ibmiaccess.ico"  + ",0";
  install_log.writeline("link3.IconLocation : "+link3.IconLocation);
  install_log.writeline(">>>ConHostPath : "+ConHostPath);
  link3.TargetPath = ConHostPath;
  install_log.writeline(">>>link3.TargetPath : "+link3.TargetPath);

  link3.WindowStyle = 3;
  link3.WorkingDirectory = prod_path;
  install_log.writeline(">>>link3.WorkingDirectory : "+link3.WorkingDirectory);
  link3.Save();
  install_log.writeline(">>>"+link3.TargetPath + " " + link3.Arguments);
}
...
// Update rss short-cut to invoke conhost
function updateRssShortcut(prod_path) {
  install_log.writeline(">>updateRssShortcut("+prod_path+")");
  install_log.writeline(">>>Check for conhost in rss shortcut.");
  rss_link = DesktopPath + "\\ACS Run Sql Statements.lnk";
  if (fso.FileExists(rss_link)) {
    install_log.writeline("rss shortcut found at: "+rss_link);
    Shel99 = new ActiveXObject("WScript.Shell");
    link3 = Shel99.CreateShortcut(DesktopPath + "\\ACS Run Sql Statements.lnk");
	install_log.writeline(">>>link3 : "+link3);
    t_path = link3.TargetPath;
	install_log.writeline(">>>link3.TargetPath : "+link3.TargetPath);
    if (t_path.toLowerCase() == ConHostPath.toLowerCase()) {
       install_log.writeline(">>>conhost found. No update needed.");
    }
       ...
       install_log.writeline(">>>Adding conhost to shortcut.");
       currentArgs = link3.Arguments;
       install_log.writeline(">>>Keeping arguments:"+currentArgs);
       // Preserve existing arguments in case they have been modified (eg -Xmx4g)
       link2.Arguments = prod_path + win_exe + " " + currentArgs;
       // link2.Description = "IBM i Access Client Solutions - Run Sql Statements";
       // link2.HotKey = "CTRL+ALT+SHIFT+S";
       // link2.IconLocation = prod_path + win_exe ;//+ ",5";
	   install_log.writeline(">>>ConHostPath : "+ConHostPath);
       link3.TargetPath = ConHostPath;
	   install_log.writeline(">>>link3.TargetPath : "+link3.TargetPath);
       ...
       link3.Save();
       install_log.writeline(">>>link3.TargetPath" + link3.TargetPath);
	   install_log.writeline(">>>link3.Arguments"+link3.Arguments);
    }
  }
  else {
    install_log.writeline(">>>rss shortcut not found.");
  }
}
